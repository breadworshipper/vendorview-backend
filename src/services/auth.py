...
from src.utils.auth import hash_password, create_access_token, verify_password, get_user, get_user_by_email, create_user
...
def register_vendor_service(street_vendor: StreetVendorCreate, db: Session):
    ...
    user_instance = User(email=street_vendor.email, name=street_vendor.name, password=hash_password(street_vendor.password), is_street_vendor=True)
    db.add(user_instance)
    db.commit()
    db.refresh(user_instance)
    
    street_vendor_instance = StreetVendor(street_vendor_name=street_vendor.street_vendor_name, street_vendor_category=street_vendor.street_vendor_category, user=user_instance.id)
    db.add(street_vendor_instance)
    db.commit()
    db.refresh(street_vendor_instance)

    return JSONResponse(status_code=201, content=street_vendor_instance.dict())
