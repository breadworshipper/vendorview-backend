...
def add_coordinates(rd: redis.Redis, key: str, lat: float, lon: float, name: str, vendor_category: StreetVendorCategoryEnum) -> None:
    """
    Add a new set of coordinates to the Redis database
    """
    rd.geoadd(key, (lon, lat, ";".join([str(name), str(vendor_category.value), str(vendor_category.name)])))
