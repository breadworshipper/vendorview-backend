...
def get_items_by_vendor(vendor_id: int, db: Session = Depends(get_db)):
    return get_items_by_vendor_service(vendor_id, db)


@items_router.delete("/delete")
def delete_items_batch(items: list[int], db: Session = Depends(get_db)):
    return delete_items_batch_service(items, db)
